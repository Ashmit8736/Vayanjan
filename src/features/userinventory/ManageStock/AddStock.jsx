...
//         </Typography>
//        // Pehle (galat hardcoded date)
// <TextField type="date" size="small" defaultValue="2026-01-23" />

// // Baad mein (automatic aaj ki date)
// <TextField 
//   type="date" 
//   size="small" 
...
import {
  ...
  MenuItem, IconButton, Paper, Divider
} from "@mui/material";
import { Add, Delete } from "@mui/icons-material";
import { useEffect, useState } from "react";
import axios from "axios";
...
const AddStock = ({onClose}) => {
  // ✅ SARI STATES ANDAR
  const [rows, setRows] = useState([{ material: "", qty: "", unit: "", comment: "" }]);
  const [rawMaterials, setRawMaterials] = useState([]);
  const [units, setUnits] = useState([]);
  const [materialSearch, setMaterialSearch] = useState(""); // ✅ ANDAR

  const token = localStorage.getItem("authToken"); // ✅ ANDAR
  ...
  const filteredMaterials = rawMaterials.filter((r) =>
    r.name.toLowerCase().includes(materialSearch.toLowerCase())
  );
  ...
  const handleMaterialChange = (index, materialId) => {
    ...
    updated[index].material = materialId;
    const selectedMat = rawMaterials.find((rm) => rm.id === Number(materialId));
    updated[index].unit = selectedMat ? selectedMat.purchase_unit_id || "" : "";
    setRows(updated);
  };
  ...
  const handleSave = async () => {
    try {
      const items = rows
        .filter((r) => r.material !== "" && r.unit !== "" && r.qty !== "" && Number(r.qty) > 0)
        .map((r) => ({
          raw_material_id: Number(r.material),
          entered_quantity: Number(r.qty),
          entered_unit_id: Number(r.unit),
      ...
      if (items.length === 0) { alert("Please add stock"); return; }

      await Promise.all(
        items.map((item) =>
          axios.post("http://localhost:5000/api/stock/stockAdd", item, {
            headers: { Authorization: `Bearer ${token}` },
          })
        )
      );
      ...
      alert("✅ All stock saved successfully");
      setRows([{ material: "", qty: "", unit: "", comment: "" }]);
    } catch (error) {
      console.error(error.response || error);
      alert("❌ Error saving stock");
    }
  ...
  return (
    <Box p={3}>
      ...
      {/* TABLE */}
      <Paper sx={{ p: 2 }}>
        <Box display="grid" gridTemplateColumns="3fr 2fr 2fr 3fr 40px" gap={2} fontWeight={600} mb={1}>
          <Typography>Raw Material</Typography>
          <Typography>Available Stock</Typography>
          <Typography>Unit</Typography>
          <Typography>Comments</Typography>
        ...
        {rows.map((row, index) => (
          <Box
            ...
            mt={2}
          >
            {/* ✅ RAW MATERIAL WITH SEARCH */}
            <Select
              size="small"
              displayEmpty
              value={row.material}
              onChange={(e) => handleMaterialChange(index, e.target.value)}
              onClose={() => setMaterialSearch("")}
            ...
            <Select size="small" displayEmpty value={row.unit} disabled>
              <MenuItem value="">Unit</MenuItem>
              {units.map((u) => (
                <MenuItem key={u.id} value={u.id}>{u.unit_name}</MenuItem>
              ))}
            </Select>
      ...
      {/* ACTION BAR */}
      <Paper sx={{ p: 2, mt: 3, background: "#fdecec", display: "flex", justifyContent: "space-between" }}>
        <Button variant="outlined" color="error" startIcon={<Add />} onClick={addRow}>
          Add New
        </Button>
        <Box>
  ...
  Cancel
          ...
          <Button variant="contained" color="error" onClick={handleSave}>Save Changes</Button>
        </Box>
      </Paper>
    </Box>
  );
